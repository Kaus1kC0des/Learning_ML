...
select pno as Project_Number,avg(hours) as Average,max(hours) as Maximum,min(hours) as Minimum
from works_on
group by pno;
...
select year(bdate) as Year,count(*)
from employee
group by year(bdate);

-- 25. Dipslay the number of projects each employee is working on
select essn,count(pno) as Counts
from works_on
group by essn;
